...
def uniform_cc(n: int, p: float, N: float, samples: int, G:nx.Graph | None = None, seed:int|np.random.Generator|None=None, fast_sampling: bool = True):
    """
    ...
    - N: Number of 2-cells (in expectation)
    - samples: Random spanning trees to sample. Larger is more accurate. Should be greater than `N`.
...
@cython.boundscheck(False)
@cython.wraparound(False)
def uniform_cc_fast(n: int, p: float, N: float, samples: int, seed:int|np.random.Generator|None=None, G:nx.Graph | None = None) -> tuple[nx.Graph, set[tuple], dict[tuple,float], dict[tuple,float]]:
    """
...
@cython.boundscheck(False)
@cython.wraparound(False)
def estimate_len_count_fast(G: nx.Graph, edges: np.ndarray[NP_EDGE], p: float, samples: int, seed: np.random.Generator) -> tuple[np.ndarray[np.float64], np.ndarray[np.int64]]:
    """
...
@cython.cpow(True)
cdef inline double calc_sampling_probability(double P, int samples, double p_c, char log_input = False) nogil:
