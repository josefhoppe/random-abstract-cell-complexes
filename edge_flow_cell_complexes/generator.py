...
class TriangulationComplexGenerator(CellComplexGenerator):
    ...
    def generate(self) -> CellComplex:
        ...
        if np.sum(can_delete_edge) == 0:
            print('WARN: cannot delete any edges')
            delete_edges = []
        elif np.sum(can_delete_edge) < self.__delete_edges:
            print(f'WARN: can only delete {np.sum(can_delete_edge)} edges, not {self.__delete_edges}')
            delete_edges = rnd.choice(edges, int(np.sum(can_delete_edge)), replace=False, p=can_delete_edge / np.sum(can_delete_edge))
        else:
        ...
        result = CellComplex(self._nodes, edges + dont_delete_edges + cells)
        ...
        while len(border_nodes) < length:
            ...
            for idx, point in enumerate(tri.simplices[next_add_simpl]):
                if point in border_nodes and not is_first:
                    ...
                    neighbor_a = tri.neighbors[next_add_simpl][(idx + 1) % 3]
                    neighbor_b = tri.neighbors[next_add_simpl][(idx + 2) % 3]
                    if (neighbor_a == -1 or (not added_simplices[neighbor_a])) and (neighbor_b == -1 or (not added_simplices[neighbor_b])):
                        is_loop = True
            ...
            if not is_loop:
                ...
                for idx, point in enumerate(tri.simplices[next_add_simpl]):
                    ...
                    neighbor_a = tri.neighbors[next_add_simpl][(idx + 1) % 3]
                    neighbor_b = tri.neighbors[next_add_simpl][(idx + 2) % 3]
                    # if both neighbors (if not outer edge, i.e. -1) are already part of the cell, we are filling a wedge.
                    if (not is_first) and neighbor_a != -1 and added_simplices[neighbor_a] and neighbor_b != -1 and added_simplices[neighbor_b]:
