...
from random_ccs import uniform_cc
...
# literal to indicate usage of random cell complex instead of CellCandidateHeuristic
RANDOM = "random"
...
class CellComplexDetectionExperiment:
    ...
    def __init__(self, cell_compl: CellComplex | CellComplexGenerator = TriangulationComplexGenerator(),
                 ...
                 search_method: CellCandidateHeuristic | Literal['ground_truth'] | Literal['random'] = CellCandidateHeuristic.MAX,
                 n_clusters: int = 11,
    ...
    def simulate_run(self, seed_num: int, flow_count: int) -> tuple[list[tuple], float, int, list[dict]]:
        ...
        random_cells = []
        if self.__search_method == RANDOM:
            G = cf.cc_to_nx_graph(current_compl)
            n = len(G.nodes)
            p = len(G.edges) * 2 / (n*n - n)
            N = 2 * self.__num_curls
            # result is G, cells, undersample, overcorrelate
            # we only need cells
            random_cells = uniform_cc(n, p, N, 1000, seed=rnd, G=G)[1]
            while(len(random_cells) < self.__num_curls):
                print(f'{len(random_cells)} / {self.__num_curls} ({N}, {n}, {p})')
                N *= 2
                random_cells = uniform_cc(n, p, N, 1000, seed=rnd, G=G)[1]
            permutation = np.random.default_rng().permutation(len(random_cells))
            sampled_cell_list = list(random_cells)
            random_cells = [sampled_cell_list[i] for i in permutation]
        flows = self.__flow_cache[seed_num][:flow_count]
        no_gradient_flows = np.copy(flows)
        for i in range(flows.shape[0]):
            no_gradient_flows[i] -= project_flow(
        ...
        while n < self.__num_curls or (approx_error > self.__approx_epsilon and self.__approx_epsilon >= 0):
            if n % 10 == 0:
                print(f'{n} / {self.__num_curls}')
            if self.__search_method == RANDOM:
                candidate_cells = [(random_cells[n],current_compl.calc_cell_boundary(random_cells[n]))]
            else:
